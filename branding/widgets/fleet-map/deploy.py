#!/usr/bin/env python3
"""
Deploy Fleet Map widget to ThingsBoard SignConnect bundle.
Idempotent: safe to re-run.

Usage:
  cd /opt/thingsboard/branding/widgets/fleet-map
  python3 deploy.py
"""
...
WIDGET_FQN = "fleet_map"
WIDGET_NAME = "Fleet Map"
...
def build_descriptor():
    default_config = json.dumps({
        "datasources": [{
            "type": "entity",
            "dataKeys": [
                {"name": "latitude", "type": "attribute", "label": "latitude", "settings": {}, "funcBody": None, "_hash": 0.1},
                {"name": "longitude", "type": "attribute", "label": "longitude", "settings": {}, "funcBody": None, "_hash": 0.2},
                {"name": "address", "type": "attribute", "label": "address", "settings": {}, "funcBody": None, "_hash": 0.3}
            ]
        }],
...
def main():
    ...
    print(f"  Type: latest")
    print(f"  Alias: All Sites / Region Sites (assetType=site)")
    print(f"  Keys: latitude, longitude, address (attributes)")
    print(f"  Size: 24 × 10")
    print(f"  Resources: Leaflet 1.9.4 (CSS + JS)")
    print("═" * 50)
